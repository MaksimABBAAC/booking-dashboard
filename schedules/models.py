...
class DailySchedule(models.Model):
    ...
    weekly_schedule = models.ForeignKey(
        WeeklySchedule, on_delete=models.CASCADE, related_name="days"
    )
    ...
    start_time = models.TimeField(
        "Начало работы", default="09:00", blank=True, null=True
    )
    end_time = models.TimeField(
        "Окончание работы", default="18:00", blank=True, null=True
    )
    ...
    break_start = models.TimeField(
        "Начало перерыва", default="12:00", blank=True, null=True
    )
    break_end = models.TimeField(
        "Конец перерыва", default="13:00", blank=True, null=True
    )
